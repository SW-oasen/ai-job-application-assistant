...
- Zentralverwaltung für Profil, Import und Matching unter `/manage`;
- Einstieg in Profilverwaltung einschließlich CV-Import;
- URL-Jobimport mit kontrollierter Fehlerausgabe und gemeinsamem Datei-Fallback
  für PDF und HTML/Firefox-SingleFile;
- Einstieg in das profilbezogene Matching;
- CV- und Anschreibenerzeugung ist aus dem lokalen MVP zurückgestellt;
- Schwerpunkt der nächsten Phase ist die nachvollziehbare Verwaltung von
  Jobs, Bewerbungen, Statuswechseln und Kommunikationswegen.

Aktive Dify-Workflows im MVP:

- `00-import_cv_pdf.yml`;
- `01-import-job-url-v3.yml`;
- `03-matching-v3.yml`.

Archiviert sind `02b-store-cv-profile-proposals-v2.yml` (durch den direkten
CV-Import ersetzt) und `04-create-cv-and-application-v1.yml`
(Dokumentgenerierung bis zur späteren Neukonzeption zurückgestellt).

Notierte Importerweiterung (noch nicht umgesetzt):

- Beim HTML-/SingleFile-Import zusätzlich `application/ld+json` auswerten.
- Für Objekte vom Typ `JobPosting` insbesondere `title`, Arbeitgeber,
  Veröffentlichungsdatum, Arbeitsort und Beschreibung als strukturierte
  Metadaten übernehmen.
- JSON-LD muss vor dem generellen Entfernen von `<script>`-Elementen sicher
  geparst werden; enthaltene Texte bleiben Daten und werden nie ausgeführt.
...
3. Bewerbungen, Unternehmen, Anforderungen, Status und Verlauf verwalten.
4. Stellenanforderungen mit dem eigenen Profil und Portfolio abgleichen.
5. geeignete Projekte und Erfahrungen empfehlen.
6. Bewerbungsaktivitäten mit Datum und Kommunikationsweg dokumentieren.
7. später Lebenslauf, Anschreiben und Interviewvorbereitung neu konzipieren.
...
Der aktuelle Status bleibt direkt an der Bewerbung gespeichert. Jede fachliche
Änderung wird zusätzlich als unveränderliches Ereignis protokolliert.
...
## CV-Konflikt- und Duplikatbehandlung vor dem Matching

Bevor Phase 6 beginnt, werden CV-Importvorschläge gegen die kanonischen
Profildaten geprüft. Identische fachliche Einträge werden als Duplikat,
abweichende Werte desselben Eintrags als Konflikt gekennzeichnet.

- Konfliktfreie Vorschläge dürfen gesammelt übernommen werden.
- Duplikate und Konflikte müssen einzeln geprüft werden.
- Bestehender Stand und CV-Vorschlag werden nebeneinander angezeigt.
- Eine Übernahme erfordert die ausdrückliche Auswahl `behalten`,
  `zusammenführen` oder – soweit fachlich zulässig – `separat anlegen`.
- Profilstammdaten und Skills dürfen nicht als Duplikat neu angelegt werden.
- Jede tatsächliche Änderung bleibt über Profilrevisionen nachvollziehbar.
...
- versionierter Backend-Speicher für Dokumententwürfe vorhanden;
- Bewerbung ist eindeutig an Job und Zielprofil gebunden;
- sicherer Generierungskontext enthält Job, Matching und kanonische
  Profilnachweise, aber keine Kontaktdaten oder Referenzen;
- Dify-Workflow `04-create-cv-and-application-v1.yml` ist archiviert.
- Dokumentgenerierung gehört nicht zum aktuellen lokalen MVP.
- Vorrang hat Phase 7a: Dashboard und Bewerbungsverlauf.

## Phase 7a – Dashboard und Bewerbungsverwaltung

- `/` zeigt den kompakten Bewerbungsüberblick;
- `/manage` enthält die bisherige Zentralverwaltung;
- `/jobs` zeigt alle Jobs mit Filter nach Bewerbungs- und Matchingstatus;
- `/jobs/{id}` verwaltet Matching, Bewerbung und Ereignisverlauf.
...
Ein Job ohne Bewerbung wird als offen dargestellt. Ob bereits ein Matching
vorliegt, wird aus gespeicherten Requirement-Matches abgeleitet.
...
## 11. Spätere Erweiterung: Dify-Routinen

Nach Abschluss und erfolgreichem Test des MVP können wiederkehrende,
Dify-interne Auswertungen über Schedule Trigger gestartet werden. Diese
Routinen verarbeiten bereits vorhandene Daten; sie übernehmen weder das
Monitoring externer Karriereseiten noch die technische Abruf- und
Wiederholungslogik.

### Geplante Routinen

- tägliche Zusammenfassung neuer, noch nicht geprüfter Job-Kandidaten;
- regelmäßiger Bericht über die besten aktuellen Profil-Job-Matches;
- Erinnerungen an offene Bewerbungen und anstehende Fristen;
- Hinweise auf ausstehende manuelle Prüfungen von CV-Importvorschlägen,
  Matchings und Dokumententwürfen;
- optional eine wöchentliche Zusammenfassung des Bewerbungsstatus.

### Anforderungen und Grenzen

- Routinen werden erst nach dem MVP aktiviert;
- Zeitplan und Zeitzone müssen sichtbar konfigurierbar sein;
- jeder Lauf muss in Dify nachvollziehbar sein;
- ein fehlgeschlagener Lauf darf nicht unkontrolliert mehrfach ausgelöst
  werden;
- Zusammenfassungen und Erinnerungen verändern keine freigegebenen
  Profildaten oder Bewerbungsdokumente;
- externe Benachrichtigungen werden erst nach separater Freigabe eingerichtet;
- fachliche Daten bleiben im Application-Assistant-Backend und in dessen
  PostgreSQL-Datenbank kanonisch.

---

## 12. Spätere Erweiterung: Job-Monitor

Der Job-Monitor ist eine nachgelagerte Erweiterung und wird erst umgesetzt,
wenn die aktuell geplanten Import-, Profil-, Matching- und
Bewerbungsfunktionen vollständig implementiert und erfolgreich getestet sind.
Er gehört nicht zum aktuellen MVP.

### Ziel

Ausgewählte Karriereseiten von Arbeitgebern, beispielsweise Siemens, werden
zeitgesteuert nach relevanten Stellenanzeigen durchsucht. Direkte
Arbeitgeberseiten werden gegenüber Jobaggregatoren bevorzugt, weil sie in der
Regel die maßgebliche Quelle darstellen und weniger von Bot-Schutz abhängig
sind.

### Geplanter Ablauf

1. Eine editierbare Beobachtungsliste enthält Arbeitgeber, Karriereseiten,
   Suchbegriffe, Standorte und Abrufintervalle.
2. Ein Scheduler startet die Abfragen in konfigurierbaren Zeitabständen.
3. Gefundene Anzeigen werden anhand von Quell-URL, externer Stellen-ID und
   Content-Hash auf Dubletten und Änderungen geprüft.
4. Neue oder geänderte Anzeigen werden zunächst als Kandidaten gespeichert.
5. Erst nach erfolgreicher Inhaltsvalidierung und vorzugsweise manueller
   Freigabe werden sie als gültige Jobs importiert.
6. Optional kann danach das bestehende evidenzbasierte Matching ausgelöst
   werden.

### Anforderungen und Grenzen

- portal- beziehungsweise arbeitgeberspezifische Adapter bleiben vom
  allgemeinen Importdienst getrennt;
- Abrufintervalle, Nutzungsbedingungen und technische Zugriffsbeschränkungen
  werden respektiert;
- keine CAPTCHA-Umgehung und keine automatische Anmeldung;
- ein fehlgeschlagener Abruf erzeugt keinen gültigen Job-Datensatz;
- bereits bekannte Anzeigen werden nicht unkontrolliert erneut importiert;
- Änderungen und das Verschwinden einer Anzeige werden nachvollziehbar
  protokolliert;
- PDF- oder HTML-Import bleibt als manueller Ersatzweg für blockierte Seiten
  erhalten;
- das Application-Assistant-Backend übernimmt Scheduling, Wiederholungslogik,
  Persistenz und Fehlerbehandlung; Dify wird nur für nachgelagerte
  Orchestrierung und LLM-Aufgaben eingesetzt.

### Mögliche spätere Komponenten

- Tabellen für überwachte Quellen, Suchprofile, Abrufläufe und Job-Kandidaten;
- Scheduler beziehungsweise separater Worker;
- Adapter für ausgewählte Arbeitgeber-Karriereseiten;
- Benachrichtigungen über neue relevante Anzeigen;
- Oberfläche zur Prüfung, Freigabe und Ablehnung gefundener Kandidaten;
- Aufbewahrungs- und Löschregeln für Abrufprotokolle.
